...
**The `/api/cron/rules` route named in the header does not exist in this repo.** `app/api/` contains `auth`, `cards`, `categories`, `chat`, `events`, `mcp`, `reports`, `reports/[id]/print`, `session` and `visuals` — no `cron`. Today this script is invoked only by `tools/build.sh` stage 6/7 and by hand.
...
3. **Mark the previous run pending:**
   ```sql
   UPDATE fired_rules SET superseded_by = 'pending'
   WHERE superseded_by IS NULL AND dismissed_at IS NULL
   ```
   Dismissed rows are deliberately excluded. *"A dismissal is the only feedback we get on whether a rule is any good, so a re-run must never silently erase one."*
...
6. **Link the superseded rows** to whatever replaced them, per child and rule:
   ```sql
...
7. **Resolve the rest:**
   ```sql
   UPDATE fired_rules SET superseded_by = NULL WHERE superseded_by = 'pending'
   ```
   *"Nothing replaced them: the rule stopped firing, which is a resolution."* A rule that stops firing therefore returns to `superseded_by IS NULL` and is counted as open again — the "open findings" count is a count of currently-true findings plus resolved-but-unlinked ones, not a strictly monotonic queue.
8. **Report:**
   ```sql
...
- **Renaming a view in `RULES`** produces an `sqlite3.OperationalError` mid-run *after* the pending sweep has already run, leaving previous rows stuck at `superseded_by = 'pending'` — a value no reader understands, and one that hides them from the `superseded_by IS NULL` filter everywhere.
