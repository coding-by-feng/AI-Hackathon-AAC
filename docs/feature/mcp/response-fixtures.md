...
> **Stale as of 2026-08-08 — the fixtures predate the current build on three further counts.** (1) They were captured when `MAX(day_local)` was 2026-08-07; the live database now reaches 2026-08-08, so every window, `data_freshness` value and windowed number shifts on regeneration. (2) `get_metrics`' `suppressed_reason` has since gained a third value, `no_data_in_window` (produced before, nothing this window); the captured responses carry only `small_sample` and `not_collected`. (3) `maya_t`'s `get_child_profile` shows `active_modes: [{ board_id: "mode_maya_t_snack", name: "Snack time", card_count: 5, position_consistency: 1 }]`, but the live `boards` table holds zero `kind = 'mode'` rows — regeneration would return `[]`. Like the `reach_gradient` divergence above, none of this is caught by CI; regenerate with `tools/gen_fixtures.py` before trusting a specific number.
