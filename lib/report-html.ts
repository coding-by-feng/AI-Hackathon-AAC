/**
 * The printable report document, as an HTML string.
 *
 * One builder, two consumers: /api/reports/[id]/print serves it as a page
 * (with the Cmd-P hint), /api/reports/[id]/pdf feeds it to headless Chrome
 * and streams back a real PDF download. Keeping them one function means the
 * paper version and the downloaded version can never drift apart.
 */
import type { ReportView } from './report-store'

export function reportPrintHtml(report: ReportView, opts: { printHint: boolean }): string {
